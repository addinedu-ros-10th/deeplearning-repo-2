...
class deviceManager(Tcp_client_manager):
    ...
    #     if (self.last_alarm == 0):
    #         return

    #     if (self.event_video_ready):
    #         print("Event Video Sending...")
    #         with open(self.file_path+f"/embedmediaStorage/{self.current_time.strftime("%Y%m%d%H%M%S")}.avi", "rb") as video:
    #             data = struct.pack("BBBIIIIIII", self.deviceManager_ID, self.situationDetector_ID, 1, *time_list, os.path.getsize(self.file_path+f"/embedmediaStorage/{self.current_time.strftime("%Y%m%d%H%M%S")}.avi"))
    #             self.send_data(data)
    ...
    #         print("Event Video Send Complete")
    #         self.event_video_ready = False
    #         self.last_alarm = 0
    #     else:
    #         return
    ...
    def alarm_set(self):
        ...
        while True:
            if self.alarm == 0:
                ...
                audio = pygame.mixer.Sound(self.file_path+'/embedmediaStorage/fire.mp3')
                print("화재용 LED")
            elif self.alarm == 2:
                audio = pygame.mixer.Sound(self.file_path+'/embedmediaStorage/fight.mp3')
                print("폭행 LED")
            elif self.alarm == 3:
                audio = pygame.mixer.Sound(self.file_path+'/embedmediaStorage/trash.mp3')
                print("무단투기 LED")
            elif self.alarm == 4:
                audio = pygame.mixer.Sound(self.file_path+'/embedmediaStorage/smoke.mp3')
                print("흡연자 LED")
            else:
            ...
            if (sound.get_busy()):
                ...
                if self.last_alarm == 0 or self.last_alarm == 255:
                    # print("패스")
                    pass
                elif self.last_alarm == 1 or self.last_alarm == 2:
                    print("알람 출력")
                    sound.play(audio)
                else:
                    sound.play(audio)
    ...
    #             # self.cap = cv2.VideoCapture("http://192.168.0.180:5100/stream?src=0")
    #             self.cap = cv2.VideoCapture("http://172.20.10.7:5100/stream?src=0")
    #             time.sleep(1)
