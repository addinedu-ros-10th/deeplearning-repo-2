...
class TcpReceiver(QThread):
    """
    백그라운드에서 TCP 소켓 통신을 통해 JSON 데이터를 수신하는 스read 클래스.
    ...
    def __init__(self, command_queue, parent=None):
        super().__init__(parent)
        self.running = True
        self.server_host = '192.168.0.86'  # situationDetector 서버 IP
        self.server_port = 2401           # situationDetector 서버 포트
            ...
            try:
                ...
                while self.running:
                    ...
                    if readable:
                        ...
                        # 버퍼에서 완전한 JSON 객체를 찾아 처리
                        while buffer:
                            try:
                                # raw_decode는 문자열에서 첫 번째 JSON 객체와 그 끝 위치를 찾아냄
                                json_data, end_pos = decoder.raw_decode(buffer)
                                self.data_received.emit(json_data)
                                
                                # 처리된 JSON 객체만큼 버퍼에서 제거
                                buffer = buffer[end_pos:].lstrip()
                            except json.JSONDecodeError:
                                # 버퍼에 아직 완전한 JSON 객체가 없으면 루프를 중단하고 데이터 추가 수신 대기
                                break
                    
                    # 데이터 송신 처리 (쓰기 가능하고, 보낼 명령이 큐에 있을 때)
                    if writable and not self.command_queue.empty():
                        try:
                            command_to_send = self.command_queue.get_nowait()
                            self.client_socket.sendall(command_to_send)
                            print(f"Command sent to server: {command_to_send.hex()}")
                        except queue.Empty:
                            pass
    ...
    def stop(self):
        ...
        if self.client_socket:
            # 소켓을 닫아 run 루프의 블로킹(recv, select)을 해제
            self.client_socket.close()
...
class PatrolDashboard(QMainWindow):
    def __init__(self):
        ...
        self.event_colors = {"화재":"red", "폭행":"orange", "쓰러진 사람":"purple", "실종자 발견":"blue", "무단 투기":"gray", "흡연자":"gray"}
        ...
        self.event_to_code_map = {
            ...
            "무단 투기": 3,
            "폭행": 4,
            # "무기": 5, # feat_detect_weapon에 해당
            "실종자 발견": 255, # 서버에 정의되지 않았으므로 임의의 값으로 설정 (해제 불가)
        }
...
#============= --- 연결 고리 2 -----=======================================        
    ...
    def process_tcp_data(self, json_data):
        ...
        새로운 데이터 구조(detection_count가 리스트ท้าย에 포함)에 맞게 수정되었습니다.
        """
        ...
        for feature_key, results in detection_features.items():
            # results가 비어있거나 리스트가 아니면 건너뜀
            if not isinstance(results, list) or not results:
                continue

            # [수정] 리스트의 마지막 요소가 detection_count인지 확인하고 분리
            detection_objects = []
            detection_count = 0
            
            # 마지막 요소가 딕셔너리이고 'detection_count' 키를 가지고 있는지 확인
            if isinstance(results[-1], dict) and "detection_count" in results[-1]:
                detection_objects = results[:-1]  # 실제 감지 객체 리스트 (마지막 요소 제외)
                detection_count = results[-1].get("detection_count", 0)
            else:
                # 만약 count 객체가 없다면, 리스트 전체를 감지 객체로 간주 (하위 호환성)
                detection_objects = results
                detection_count = len(results)

            # 감지된 객체가 없으면 이 feature에 대한 처리 중단
            if detection_count == 0:
                continue
            
            # [수정] 실제 감지 객체만 담긴 'detection_objects' 리스트를 핸들러로 전달
            if feature_key == "feat_detect_fire":
                self.handle_generic_event(detection_objects, "화재", "confidence")
            
            elif feature_key == "feat_detect_fall":
                self.handle_generic_event(detection_objects, "쓰러진 사람", "score_event")

            elif feature_key == "feat_detect_violence":
                self.handle_violence_event(detection_objects)
            
            elif feature_key == "feat_detect_missing_person":
                self.handle_missing_person_event(detection_objects)

            elif feature_key == "feat_detect_smoke":
                self.handle_generic_event(detection_objects, "흡연", "confidence", target_class="smoke")

            elif feature_key == "feat_detect_trash":
                self.handle_trash_event(detection_objects)
