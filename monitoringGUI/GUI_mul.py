...
class PatrolDashboard(QMainWindow):
    def __init__(self):
        ...
        self.dumping_model = self.smoker_model = None # 모델 파일이 없으면 시뮬레이션만 동작
    ...
    def setupUi(self, MainWindow):
        MainWindow.setWindowTitle("AURA 관제 대시보드 (통합 시뮬레이터)")
        ...
        # Left: Video Panels (Tab Widget)
        self.video_tabs = QTabWidget()
        self.main_layout.addWidget(self.video_tabs, 7) # 70%
        
        self.video_widgets = {}
        video_panel_names = ["메인 순찰","폭행","화재","누워있는 사람","실종자","무단 투기","흡연자"]
        for name in video_panel_names:
            widget = AspectRatioWidget()
            self.video_tabs.addTab(widget, name)
            self.video_widgets[name] = widget

        # Right: Control Panel
        control_panel = QWidget()
        self.control_layout = QVBoxLayout(control_panel)
        self.main_layout.addWidget(control_panel, 3) # 30%
        ...
        # Log Panels (Tab Widget)
        self.log_tabs = QTabWidget()
        self.log_browsers = {}
        log_panel_names = ["통합 로그", "폭행", "화재", "누워있는 사람", "실종자", "무단 투기", "흡연자"]
        for name in log_panel_names:
            browser = QTextBrowser()
            self.log_tabs.addTab(browser, name)
            self.log_browsers[name] = browser
        self.control_layout.addWidget(self.log_tabs)
    ...
    def update_frame(self):
        ...
        for widget in self.video_widgets.values():
            widget.video_label.setPixmap(pixmap)
            
        self.frame_buffer.append(self.preprocess_frame(frame)); self.frame_counter+=1
        if self.frame_counter >= 5: # Inference every 5 frames
            if self.dumping_model: self.run_inference(self.dumping_model, "무단 투기")
            if self.smoker_model: self.run_inference(self.smoker_model, "흡연자")
            self.frame_counter = 0
    ...
    def run_inference(self, model, event_type):
        if len(self.frame_buffer) < 16: return
        clip=torch.stack(list(self.frame_buffer),dim=1).unsqueeze(0).to(self.device)
        with torch.no_grad(): output=model(clip); prob=torch.sigmoid(output.view(-1)).item()
        if prob > 0.6: # 높은 확률일 때만 이벤트 발생
    ...
    def trigger_event(self, event_type, prob=None, is_auto=False):
        ...
        self.log_browsers[event_type].append(html_log)
        self.log_browsers["통합 로그"].append(html_log)
        ...
        for r in range((screen_rect.height() - margin) // (d_h + margin)):
            for c in range((screen_rect.width() - margin) // (d_w + margin)):
                ...
                if (x, y) not in positions:
                    dialog.move(x, y); return
        dialog.move(margin, margin) # Fallback
    ...
    def resolve_alert(self, event_type):
        ...
        self.log_browsers["통합 로그"].append(log_message); self.log_entries.append({"timestamp":timestamp, "message":f"{event_type} 상황 종료됨"})
