...
def receive_processed_video(gui_video_queue: queue.Queue, gui_db_queue: queue.Queue, shutdown_event: threading.Event):
    ...
    db_data_buffer = collections.deque(maxlen=100)

    try:
        while not shutdown_event.is_set():
            try:
                # select로 AI/DB 모두 감시
                readable, _, _ = select.select([ai_sock, db_sock], [], [], 0.5)
                ...
                for sock in readable:
                    ...
                    elif sock is db_sock:
                        db_packet, _ = db_sock.recvfrom(65535)
                        try:
                            db_data = db_packet.decode('utf-8')
                            db_data_buffer.append(db_data)
                            gui_db_queue.put(db_data)
                        except Exception as e:
                            print(f"[DB] 데이터 디코딩 실패: {e}")
