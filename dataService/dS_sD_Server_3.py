...
def handle_client(conn, addr):
    ...
    try:
        while True:
            ...
            while True:
                try:
                    ...
                    if "event_video_meta" in json_obj:
                    # detection 데이터 수신 → DB에는 아직 저장 안 함
                        # print(json_obj)
                        detection_data = json_obj
                        patrol_number = detection_data.get('patrol_number', 'Unknown')
                        ...
                        if len(detection_dict) != 0:
                            print("[INFO] detection_dict 존재함", detection_dict)
                        else:
                            # 10초마다 상태 출력
                            now = time.time()
                            if now - last_log_time >= 10:
                                print("[INFO] detection_dict 비어있음!!!!!!")
                                last_log_time = now
                            return
                        ...
                        # print(json_obj)
                    # 영상 데이터 처리
                    # 영상정보가 메타정보보다 먼저 도착한 패킷인 경우

                        # if not video_file or not event_dir:
                        #     print("[ERROR] Video data received before detection data. Skip processing.")
                        #     return              # 함수 종료 → 이후 코드 실행 안 함
                    # if not video_file:  
                    #     if not event_dir:
                    ...
                    if video_content == 'DONE':
                        print("[INFO] Video 수신 완료!!!!.")
                        if buffer:  # ✅ buffer에 데이터가 있는 경우만 저장
                            with open(video_path, "wb") as f:
                                f.write(buffer)
                            print(f"[INFO] Video saved successfully at {video_path}")
                        else:
                            print("[WARN] Video buffer is empty, nothing saved!")
                    ...
                    #         print(f"[INFO] Video saved successfully at {video_path}")
                            ...
                            if detection_data:
                                # detection + video 함께 DB 저장
                                save_to_db(detection_data, video_path=video_path, mime_type="video/mp4")
                            
                            else:
                                print("[WARN] Video received but no detection metadata, DB situation skipped.")
        ...
        if video_file:
            ...
            # 영상이 아예 안 온 경우 situation 테이블만 저장
            save_to_db(detection_data)
