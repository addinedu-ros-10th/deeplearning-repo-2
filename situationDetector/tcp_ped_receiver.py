...
def handle_tcp_client(shared_metadata:dict, metadata_lock: threading.Lock, shutdown_event: threading.Event):
    ...
    print(f"AI Server (TCP) : 서버가 {TCP_HOST}:{TCP_PORT}에서 연결 대기")

    while not shutdown_event.is_set():
        try:
            conn, addr = server_sock.accept()
            with conn:
                print(f"AI Server (TCP) : 클라이언트 연결됨: {addr}")
                while not shutdown_event.is_set():
                    # 데이터 수신 (4096 바이트 버퍼)
                    data = conn.recv(4096)
                    if not data:
                        print(f"AI Server (TCP) : 클라이언트 연결 끊어짐: {addr}")
                        break
            ...
            print(f"AI Server (TCP) : 서버 오류: {e}")
            break
    
    server_sock.close()
    print("AI Server (TCP) : 수신 스레드를 종료합니다.")